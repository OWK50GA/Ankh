...
export default function ConfigurationForm({ contractData, accountInfo }) {
    ...
    const [formInputValues, setFormInputValues] = useState({
        ...
        account: accountInfo.walletAddress
    });
    const [form, setForm] = useState(() => getInitialFormState(constructor));
    ...
    const inputElements = constructor.inputs.map((input, index) => {
        ...
        return (<ContractInput abi={contractData.abi} setForm={setForm} form={form} stateObjectKey={key} paramType={input} setFormErrorMessage={setFormErrorMessage}/>);
    });
    ...
    const handleDeploy = async () => {
        ...
        try {
            ...
            const contract = await deployContract(contractData, accountInfo.rpcUrl, accountInfo.walletAddress, accountInfo.privateKey, calldata);
            if (!contract) {
                throw new Error("Deployment unsuccessful");
            }
            console.log(contract);
            // setContractAddress((contract.address as `0x${string}`))
            setContractFunctionsData(prev => ({ ...prev, contractAddress: contract.address }));
            setContractData(prev => ({ ...prev, contractAddress: contract.address }));
        }
        catch (err) {
    ...
    const handleLoadContract = () => {
        if (!formInputValues.contractAddress || formInputValues.contractAddress === "") {
    ...
    return (<div className="bg-[#161616] rounded-2xl px-4 py-4 flex flex-col gap-10 w-fit h-fit">
    ...
    readOnly value={formInputValues.network} onChange={(e) => {
            setFormInputValues(prev => ({ ...prev, network: e.target.value }));
    ...
    readOnly value={formInputValues.rpcUrl} onChange={(e) => {
    ...
    readOnly value={formInputValues.account} onChange={(e) => {
            setFormInputValues(prev => ({ ...prev, account: e.target.value }));
        }} className="outline-none w-[300px]"/>
                        </span>
                    </div>
    ...
    readOnly value={formInputValues.classHash} onChange={(e) => {
            setFormInputValues(prev => ({ ...prev, classHash: e.target.value }));
                    ...
                    <div className="flex-col flex gap-1.5">
                        ...
                        <span className="flex border bg-[#1e1e1e] items-center justify-start gap-4 h-10 rounded-lg p-4 focus-within:border-[#d57b52]">
                            <input type="text" value={formInputValues.contractAddress} onChange={(e) => {
            setFormInputValues(prev => ({ ...prev, contractAddress: e.target.value }));
        }} className="outline-none w-[300px]"/>
                        </span>
                    </div>
                </div>
                ...
                <div className="flex justify-end gap-2.5">
                    <button className="bg-gradient-to-r from-[#9433DC] to-[#D57B52] cursor-pointer px-4 py-2.5 rounded-lg" onClick={handleLoadContract}>
                        Load Contract
                    </button>
                    <button className="bg-gradient-to-r from-[#9433DC] to-[#D57B52] cursor-pointer px-4 py-2.5 rounded-lg" onClick={() => { }}>
                        Save Settings
                    </button>
                </div>
            </form>
            ...
            "tooltip before:content-[attr(data-tip)] before:right-[-10px] before:left-auto before:transform-none"}`} data-tip={`${getTopErrorMessage(formErrorMessage)}`}>
                    <button className="btn bg-gradient-to-r from-[#9433DC] to-[#D57B52] shadow-none border-none text-white px-4 py-2.5 rounded-md mt-3" onClick={() => handleDeploy()} disabled={deploying}>
