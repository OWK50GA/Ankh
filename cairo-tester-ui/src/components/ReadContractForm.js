...
export default function ReadContractForm({ contractAddress, abiFunction, abi }) {
    ...
    const inputElements = abiFunction.inputs.map((input, index) => {
        ...
        return (<ContractInput abi={abi} key={key} form={form} setForm={setForm} stateObjectKey={key} paramType={input} setFormErrorMessage={setFormErrorMessage}/>);
    });
    async function handleRead() {
        const newInputValue = getArgsAsStringInputFromForm(form);
        const expectedArgCount = abiFunction.inputs.length;
        const isValid = Array.isArray(newInputValue) &&
    ...
    return (<div className="flex flex-col gap-3 py-5 first:pt-0 last:pb-1">
            ...
            <div className="flex justify-between gap-2 flex-wrap">
                <div className="grow w-4/5">
                    {data !== null && data !== undefined && (<div className="bg-input text-sm px-4 py-1.5 break-words">
            ...
            "tooltip before:content-[attr(data-tip)] before:right-[-10px] before:left-auto before:transform-none"}`} data-tip={`${getTopErrorMessage(formErrorMessage)}`}>
                    <button className="btn bg-gradient-to-r from-[#9433DC] to-[#D57B52] btn-sm shadow-none border-none text-white px-2 py-2 rounded-md disabled:cursor-not-allowed" onClick={handleRead} disabled={(inputValue) || isError(formErrorMessage) || !readyForInteraction}>
                        {inputValue && (<span className="loading loading-spinner loading-xs"></span>)}
                        Read 📡
                    </button>
                </div>
            </div>
...
//# sourceMappingURL=ReadContractForm.js.map