...
const ContractInterface: React.FC = () => {
    const [contract, setContract] = useState<ContractArtifact | null>(null);
    ...
    const initializeFunctionInputs = (contractData: ContractArtifact) => {
    ...
    const callFunction = (functionName: string) => {
    ...
    const updateFunctionInput = (functionName: string, index: number, value: string) => {
    ...
    const getFunctionsByType = (type: string) => {
        if (!contract?.abi) return [];
        return contract.abi.filter(func => 
            func.type === 'function' && 
    ...
    return (
        <div className="container">
            ...
            <div className="section deployment-section">
                ...
                {(deploymentInfo.classHash || deploymentInfo.contractAddress) && (
                    <div className="deployment-info">
                        {deploymentInfo.classHash && (
            ...
            <div className="functions-container">
                <div className="functions-column">
                    <div className="section">
                        ...
                        <div className="functions-list">
                            {getFunctionsByType('view').map((func, index) => (
                                <FunctionCard
                                    key={`view-${index}`}
                                    func={func}
                                    inputs={functionInputs[func.name] || []}
                                    onInputChange={(index, value) => updateFunctionInput(func.name, index, value)}
                                    onCall={() => callFunction(func.name)}
                                    isLoading={isLoading[func.name]}
                                    disabled={!deploymentInfo.contractAddress}
                                    type="view"
                                />
                            ))}
                        </div>
                ...
                <div className="functions-column">
                    <div className="section">
                        ...
                        <div className="functions-list">
                            {getFunctionsByType('write').map((func, index) => (
                                <FunctionCard
                                    key={`write-${index}`}
                                    func={func}
                                    inputs={functionInputs[func.name] || []}
...
const FunctionCard: React.FC<FunctionCardProps> = ({
    ...
    return (
        <div className={`function-card ${type}`}>
            ...
            <div className="function-footer">
                <button
                    className={`button ${type === 'view' ? 'secondary' : 'primary'} ${isLoading ? 'loading' : ''}`}
                    onClick={onCall}
            ...
            {func.outputs && func.outputs.length > 0 && (
                <div className="function-outputs">
                    <small>Returns: {func.outputs.map((output: any) => output.type).join(', ')}</small>
                </div>
            )}
        </div>
    );
