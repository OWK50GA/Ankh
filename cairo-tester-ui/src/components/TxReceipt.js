...
import { decodeContractResponse } from "../utils";
// import {
//   CheckCircleIcon,
...
export const TxReceipt = (txResult) => {
    ...
    return (<div className="flex text-sm rounded-[5px] peer-checked:rounded-b-none min-h-0 bg-input py-0">
      <div className="mt-1 pl-2">
        {txResultCopied ? (<CheckCircleIcon className="ml-1.5 text-xl font-normal text-sky-600 h-5 w-5 cursor-pointer" aria-hidden="true"/>) : (
        //@ts-ignore component works but some typing issue came up, ts-expect-error does not work
        <CopyToClipboard text={decodeContractResponse({
                resp: txResult,
                abi: [],
                functionOutputs: [],
                asText: true,
...
//# sourceMappingURL=TxReceipt.js.map