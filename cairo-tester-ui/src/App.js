...
function App() {
    ...
    useEffect(() => {
        ...
        const handleContractDataMessage = (event) => {
        ...
        const handleAccountInfoMessage = (event) => {
            const message = event.data;
    ...
    return (<StarknetProvider>
      {/* <CairoTesterProvider> */}
        ...
        <div className='w-full p-4 bg-[#161616] text-[#9BDBFF] min-h-screen'>
          <h1 className="text-xl font-bold mb-4">{contractData ? `${contractData.name}` : 'VS Code Interaction'}</h1>
          
          {(contractDataLoading) && (<div className="flex items-center justify-center p-8">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-500"></div>
              <span className="ml-2">Loading contract data && account info...</span>
            </div>)}
          
          {error && (<div className="p-4 bg-red-900 text-white rounded-md">
              Error: {error}
            </div>)}
          
          {contractData && !contractDataLoading && !accountInfoLoading && (<div className='flex flex-col gap-12'>
              <ConfigurationForm contractData={contractData} accountInfo={accountInfo || {
                privateKey: "",
                walletAddress: "",
                rpcUrl: ""
            }}/>
          ...
          {!contractData && !contractDataLoading && !error && (<div className="p-4 bg-yellow-900 text-white rounded-md">
