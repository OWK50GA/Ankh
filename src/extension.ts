...
export function activate(context: vscode.ExtensionContext) {
  ...
  const closeAllPanelsCommand = vscode.commands.registerCommand(
    "ankh.closeAllPanels",
    () => {
  ...
  const showActivePanelsCommand = vscode.commands.registerCommand(
    "ankh.showActivePanels",
    () => {
      ...
      if (activePanels.length === 0) {
        ...
        const panelNames = activePanels.map((panel) =>
          panel.getContractName(panel),
        );
        vscode.window.showInformationMessage(
          `Active panels: ${panelNames.join(", ")}`,
        );
      }
    },
  );

  const selectWorkspaceRootCommand = vscode.commands.registerCommand(
    "ankh.selectWorkspaceRoot",
    async () => {
  ...
  const showWorkspaceRootCommand = vscode.commands.registerCommand(
    "ankh.showWorkspaceRoot",
    () => {
      ...
      if (currentRoot && workspaceFolders) {
        ...
        vscode.window.showInformationMessage(
          `Cairo workspace root: ${relativePath || "workspace root"}`,
        );
      } else {
